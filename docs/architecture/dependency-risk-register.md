...
| [DR-002](https://github.com/seborbie/talos-community/issues/3)       | Vendored Samsa -> rsasl -> core2; Samsa/Kafka -> rand 0.8           | RUSTSEC-2026-0105; `core2` is also yanked. The rand finding RUSTSEC-2026-0097 is fixed by the locked 0.8.6 update                                                                                                                                  | The telemetry consumer uses a pinned, locally patched Samsa because the upstream crate is no longer sufficient for Talos. The previously affected rand 0.8 line is now locked to patched 0.8.6. Kafka input remains authenticated and validated.                                                                                                    | Replace vendored Samsa/legacy Kafka dependencies with a maintained client, then remove `core2`, old rand, and this entry.                                                                                    |
