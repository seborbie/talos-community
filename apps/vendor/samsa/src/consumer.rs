...
impl<'a, T: BrokerConnection + Clone + Debug + 'a> Consumer<T> {
    ...
    pub fn into_autocommit_stream(
        ...
        try_stream! {
            let mut last_committed_offsets: Option<PartitionOffsets> = None;
            for await stream_message in self.stream() {
                let (messages, offsets) = stream_message?;
                let should_commit = last_committed_offsets
                    .as_ref()
                    .map_or(true, |last| last != &offsets);
                yield messages;
                if should_commit {
                    ...
                    last_committed_offsets = Some(offsets);
                } else {
                    tracing::debug!("Skipping offset commit; offsets have not advanced");
                }
            }
        }
    }
