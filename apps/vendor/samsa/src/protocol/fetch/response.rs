...
fn parse_partition(s: NomBytes) -> IResult<NomBytes, Partition> {
    ...
    let (s, record_batch) = if records_len <= 0 {
        (s, vec![])
    } else {
        // Keep record-batch parsing scoped to this partition's records payload.
        // Without this boundary, an empty records section can accidentally parse into
        // the next topic bytes and corrupt the response stream.
        let (s, records) = take(records_len as usize)(s)?;
        let (_, record_batch) = many0(parse_record_batch)(NomBytes::new(records.into_bytes()))?;
        (s, record_batch)
    };
