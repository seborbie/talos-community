...
public final class PermissionFlowController: ObservableObject {
    ...
    public func authorize(
        ...
        TalosPermissionFlowDebugLog.write(
            "authorize_entered pane=\(pane) suggestedAppURLs=\(suggestedAppURLs.map { $0.path }.joined(separator: ",")) sourceFrame=\(TalosPermissionFlowDebugLog.describe(sourceFrameInScreen)) frontmost=\(NSWorkspace.shared.frontmostApplication?.bundleIdentifier ?? "nil")"
        )
        TalosPermissionFlowDebugLog.write("authorize_before_closeOtherActivePanel")
        closeOtherActivePanelIfNeeded()

        TalosPermissionFlowDebugLog.write("authorize_before_rememberPreviousFrontmostApplication")
        rememberPreviousFrontmostApplication()
        currentPane = pane
        pendingLaunchSourceFrame = sourceFrameInScreen
        TalosPermissionFlowDebugLog.write("authorize_before_configureSuggestedApps")
        configureSuggestedApps(with: suggestedAppURLs)
        TalosPermissionFlowDebugLog.write(
            "authorize_before_system_settings_open pane=\(pane) url=\(pane.settingsURL.absoluteString)"
        )
        let didOpenSettings = SystemSettings.open(url: pane.settingsURL)
        TalosPermissionFlowDebugLog.write(
            "authorize_after_system_settings_open pane=\(pane) didOpenSettings=\(didOpenSettings)"
        )

        Self.activeController = self
        TalosPermissionFlowDebugLog.write("authorize_before_showPanel")
        showPanel()
        TalosPermissionFlowDebugLog.write("authorize_after_showPanel")
        TalosPermissionFlowDebugLog.write("authorize_before_tracker_startTracking")
        tracker.startTracking(promptIfNeeded: configuration.promptForAccessibilityTrust)
        TalosPermissionFlowDebugLog.write("authorize_after_tracker_startTracking")
    }
    ...
    public func showPanel() {
        if panel == nil {
            TalosPermissionFlowDebugLog.write("showPanel_before_create_FloatingDropPanel")
            panel = FloatingDropPanel(controller: self)
            TalosPermissionFlowDebugLog.write("showPanel_after_create_FloatingDropPanel")
        }
        ...
        if let settingsFrame = tracker.currentFrame {
            TalosPermissionFlowDebugLog.write(
                "showPanel_present_with_existing_settingsFrame settingsFrame=\(TalosPermissionFlowDebugLog.describe(settingsFrame))"
            )
        ...
        if let sourceFrame = pendingLaunchSourceFrame {
            TalosPermissionFlowDebugLog.write(
                "showPanel_show_at_sourceFrame sourceFrame=\(TalosPermissionFlowDebugLog.describe(sourceFrame))"
            )
            panel.show(at: sourceFrame)
        } else {
            TalosPermissionFlowDebugLog.write("showPanel_center_and_show")
            panel.center()
            panel.show()
        }
    }
    ...
    public func closePanel(returnToPreviousApp: Bool = false) {
        TalosPermissionFlowDebugLog.write("closePanel_entered returnToPreviousApp=\(returnToPreviousApp) hasPanel=\(panel != nil)")
    ...
    /// Replaces the active panel list when a host supplies explicit targets.
    func configureSuggestedApps(with urls: [URL]) {
        TalosPermissionFlowDebugLog.write("configureSuggestedApps_entered count=\(urls.count)")
        let uniqueURLs = urls.uniqueAppURLs()
        if uniqueURLs.isEmpty {
            mergeDroppedApps(with: [])
            return
        }

        let previousPaths = Set(droppedApps.map { $0.path })
        droppedApps = uniqueURLs
        for url in uniqueURLs where previousPaths.contains(url.path) == false {
            onDrop?(url)
        }
        TalosPermissionFlowDebugLog.write(
            "configureSuggestedApps_completed droppedApps=\(droppedApps.map { $0.path }.joined(separator: ","))"
        )
    }

    /// Merges unique app bundle URLs into the current panel list.
    func mergeDroppedApps(with urls: [URL]) {
        TalosPermissionFlowDebugLog.write("mergeDroppedApps_entered count=\(urls.count)")
        for url in urls.uniqueAppURLs() {
            registerDroppedApp(url)
        }
        TalosPermissionFlowDebugLog.write(
            "mergeDroppedApps_completed droppedApps=\(droppedApps.map { $0.path }.joined(separator: ","))"
        )
    }
    ...
    private func presentPanel(_ panel: FloatingDropPanel?, for settingsFrame: CGRect) {
        TalosPermissionFlowDebugLog.write(
            "presentPanel_entered hasPanel=\(panel != nil) settingsFrame=\(TalosPermissionFlowDebugLog.describe(settingsFrame)) pendingSourceFrame=\(TalosPermissionFlowDebugLog.describe(pendingLaunchSourceFrame))"
        )
        guard let panel else { return }

        if let sourceFrame = pendingLaunchSourceFrame {
            TalosPermissionFlowDebugLog.write("presentPanel_before_panel_present")
            panel.present(from: sourceFrame, to: settingsFrame)
            pendingLaunchSourceFrame = nil
        } else {
            TalosPermissionFlowDebugLog.write("presentPanel_before_panel_snap")
            panel.snap(to: settingsFrame)
        }
        TalosPermissionFlowDebugLog.write("presentPanel_completed")
    }
