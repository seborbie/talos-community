...
final class SettingsWindowTracker {
    ...
    func startTracking(promptIfNeeded: Bool) {
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_startTracking_entered promptIfNeeded=\(promptIfNeeded)")
        if promptIfNeeded {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_startTracking_before_requestAccessibilityTrust")
            requestAccessibilityTrust()
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_startTracking_after_requestAccessibilityTrust")
        }

        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_startTracking_before_stopTracking")
        stopTracking()

        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_startTracking_before_timer")
        ...
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_startTracking_before_attachIfNeeded")
        attachIfNeeded()
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_startTracking_completed")
    }

    /// Tears down polling and all AX observers so a future tracking session
    /// begins from a clean state.
    func stopTracking() {
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_stopTracking_entered hasPollTimer=\(pollTimer != nil) hasAppObserver=\(appObserver != nil) hasWindowObserver=\(windowObserver != nil)")
        ...
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_stopTracking_completed")
    }
    ...
    private func attachIfNeeded() {
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_entered")
        guard let app = runningSettingsApplication() else {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_no_running_settings")
        ...
        missingAppPollCount = 0
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_found_settings pid=\(app.processIdentifier) activationPolicy=\(app.activationPolicy.rawValue)")

        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_before_updateFrameFromWindowServer")
        updateFrameFromWindowServer(for: app.processIdentifier)
        let axTrusted = AXIsProcessTrusted()
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_after_updateFrameFromWindowServer axTrusted=\(axTrusted) currentFrame=\(TalosPermissionFlowDebugLog.describe(currentFrame))")
        guard axTrusted else { return }

        let appElement = AXUIElementCreateApplication(app.processIdentifier)

        if appObserver == nil {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_before_make_app_observer")
            ...
            if let appObserver {
                ...
                TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_added_app_observer")
            } else {
                TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_make_app_observer_failed")
            }
        }

        guard let window = mainWindow(for: appElement) else {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_no_main_window")
            return
        }
        guard isSameElement(window, observedWindow) == false else {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_same_window_before_updateCurrentFrame")
            updateCurrentFrame()
            return
        }

        observedWindow = window
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_before_updateWindowObserver")
        updateWindowObserver(for: app.processIdentifier, window: window)
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_before_updateCurrentFrame")
        updateCurrentFrame()
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_attachIfNeeded_completed currentFrame=\(TalosPermissionFlowDebugLog.describe(currentFrame))")
    }
    ...
    private func updateFrameFromWindowServer(for pid: pid_t) {
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateFrameFromWindowServer_entered pid=\(pid)")
        guard let frame = windowServerFrame(for: pid) else {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateFrameFromWindowServer_no_frame")
            return
        }
        guard currentFrame != frame else {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateFrameFromWindowServer_unchanged frame=\(TalosPermissionFlowDebugLog.describe(frame))")
            return
        }
        currentFrame = frame
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateFrameFromWindowServer_frame_changed frame=\(TalosPermissionFlowDebugLog.describe(frame))")
        onFrameChange?(frame)
    }
    ...
    private func updateWindowObserver(for pid: pid_t, window: AXUIElement) {
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateWindowObserver_entered pid=\(pid)")
        ...
        if let windowObserver {
            ...
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateWindowObserver_added")
        } else {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateWindowObserver_makeObserver_failed")
        }
    }
    ...
    private func updateCurrentFrame() {
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateCurrentFrame_entered")
        guard let window = observedWindow else {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateCurrentFrame_no_window")
            return
        }
        guard
            ...
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateCurrentFrame_missing_position_or_size")
            return
        }

        let frame = appKitFrame(fromGlobalTopLeftFrame: CGRect(origin: position, size: size))
        guard currentFrame != frame else {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateCurrentFrame_unchanged frame=\(TalosPermissionFlowDebugLog.describe(frame))")
            return
        }
        currentFrame = frame
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_updateCurrentFrame_changed frame=\(TalosPermissionFlowDebugLog.describe(frame))")
        onFrameChange?(frame)
    }
    ...
    private func makeObserver(for pid: pid_t) -> AXObserver? {
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_makeObserver_entered pid=\(pid)")
        ...
        guard result == .success else {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_makeObserver_failed pid=\(pid) result=\(result.rawValue)")
            return nil
        }
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_makeObserver_succeeded pid=\(pid)")
        return observer
    }
    ...
    private func windowServerFrame(for pid: pid_t) -> CGRect? {
        TalosPermissionFlowDebugLog.write("SettingsWindowTracker_windowServerFrame_entered pid=\(pid)")
        guard
            let windows = CGWindowListCopyWindowInfo([.optionOnScreenOnly, .excludeDesktopElements], kCGNullWindowID)
                as? [[String: Any]]
        else {
            TalosPermissionFlowDebugLog.write("SettingsWindowTracker_windowServerFrame_no_window_list")
            return nil
        }
        ...
        TalosPermissionFlowDebugLog.write(
            "SettingsWindowTracker_windowServerFrame_completed windowCount=\(windows.count) bestMatch=\(TalosPermissionFlowDebugLog.describe(bestMatch))"
        )
        return bestMatch
    }
